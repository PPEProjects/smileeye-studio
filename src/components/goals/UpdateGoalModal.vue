<template>
  ...
  >
    <a-form :model="formState" layout="vertical" @finish="updateHandle">
      ...
      <a-form-item>
        <a-image :src="$cdn(formState.image?.[0])"/>
      </a-form-item>
      <a-form-item>
        <div class="flex justify-end">
          ...
          >
            <a-button type="danger" :disabled="loading">
              {{ t('goals.form.upsert.actions.cancel') }}
            </a-button>
          </a-popconfirm>

          <a-button type="primary" class="ml-auto" ghost :disabled="loading">
            {{ t('goals.form.upsert.actions.detail') }}
          </a-button>
          ...
          <a-button
              ...
              :loading="loading"
          >
            {{ t('goals.form.upsert.actions.add') }}
          </a-button>
        </div>
      </a-form-item>
<!--      <pre>{{ formState }}</pre>-->
...
// Update
const {mutate: updateAction, loading} = useMutation<UpdateUserInfo,
    UpdateUserInfoVariables>(UPDATE_USER_INFO)
const updateHandle = () => {
