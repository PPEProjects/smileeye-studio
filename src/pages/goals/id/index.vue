<template>
  ...
  <a-spin :spinning="loading">
    <div
      v-if="!Object.keys(template).length && isConfirmed"
      class="h-[200px]"
    ></div>
    ...
    <div v-else>
      <template v-if="isConfirmed">
          ...
          <div class="w-1/2 flex-shrink-0 pl-4">
            ...
            <div class="mt-4">
              <div class="text-xs text-gray-500 text-center">
                Biểu đồ học sinh của {{ template.goal.name }}
              </div>
              <div class="text-xs text-gray-500 text-center mt-4">
                <a-tag color="#9c27b0">
                  {{ t('payment.status.in_need') }}
                  {{ Math.round(template.detail_template.percent_in_need) }} %
                </a-tag>

                <a-tag color="#87d068">
                  {{ t('payment.status.trial') }}
                  {{ Math.round(template.detail_template.percent_trial) }} %
                </a-tag>

                <a-tag color="#f50">
                  {{ t('payment.status.on_buy') }}
                  {{ Math.round(template.detail_template.percent_on_buy) }} %
                </a-tag>

                <a-tag color="#3b66f5">
                  {{ t('payment.status.paid_confirmed') }}
                  {{
                    Math.round(template.detail_template.percent_paid_confirmed)
                  }}
                  %
                </a-tag>
              </div>
        ...
        <div class="mt-4">
          <tabs-animation
            ...
            class="flex items-center h-full border-b"
          >
            <button
            ...
            <button
              class="ml-3 block goal-tab px-2 goal-tab"
              :class="{
                _active: $route.query.tab === 'meetings'
              }"
              @click="$router.replace({ query: { tab: 'meetings' } })"
            >
              Meetings
            </button>
          </tabs-animation>
      ...
      <template v-else>
        <div class="mt-5 overflow-hidden">
          <goal-tree :goal="Number($route.params.id)" />
        </div>
      </template>
    </div>
  </a-spin>
...
const EditRuleModal = defineAsyncComponent(
  () => import('@components/users/EditRuleModal.vue')
)
...
const UserCellInfo = defineAsyncComponent(
  () => import('@components/user/UserCellInfo.vue')
)
const GoalTree = defineAsyncComponent(
  () => import('@components/workspace/GoalTree.vue')
)
const StudentsTable = defineAsyncComponent(
  () => import('@components/goals/StudentsTable.vue')
)
const TeachersTable = defineAsyncComponent(
  () => import('@components/goals/TeachersTable.vue')
)
const AddCoachModal = defineAsyncComponent(
  () => import('@components/goals/AddCoachModal.vue')
)
