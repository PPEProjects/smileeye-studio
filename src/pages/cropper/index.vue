<template>
  ...
  <header @dragover.prevent v-cloak @drop.prevent="onDropImage" class="border-dashed border-2 border-gray-400 py-12 flex flex-col justify-center items-center">
    <p class="mb-3 font-semibold text-gray-900 flex flex-wrap justify-center">
      <span>Drag and drop your</span>&nbsp;<span>files anywhere or</span>
    </p>
    <input id="hidden-input" type="file" accept="images/*" class="hidden" @change="selectImage" />
    <label for="hidden-input" class="mt-2 rounded-sm cursor-pointer px-3 py-1 bg-gray-200 hover:bg-gray-300 focus:shadow-outline focus:outline-none">
      Upload a file
    </label>
  </header>
  ...
  >
    ...
    <div class="w-[250px] flex items-center justify-around mx-auto mt-5">
      ...
      <button @click="$refs.cropperRef.rotate(-90)" class="border w-8 h-8 flex items-center justify-center rounded border-gray-200 text-gray-600">
        <svg class="fill-current" width="15" height="15">
          <use xlink:href="#i-rotate-left"></use>
        </svg>
      </button>

      <button @click="$refs.cropperRef.relativeZoom(-0.2)" class="border w-8 h-8 flex items-center justify-center rounded border-gray-200 text-gray-600">
